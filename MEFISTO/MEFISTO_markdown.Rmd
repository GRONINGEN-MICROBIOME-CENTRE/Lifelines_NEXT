...
# load model and data ----
load(here("run_10FAC_BAC_NOGRP_14:59:36_26_01_2024_copy/MOFAobject_untrained_26_01_2024.rda"))
load(here("run_10FAC_BAC_NOGRP_14:59:36_26_01_2024_copy/feature_matrix_26_01_2024.rda"))
load(here("run_10FAC_BAC_NOGRP_14:59:36_26_01_2024_copy/data_for_MOFA_26_01_2024.rda"))

MOFAobject <- load_model(here("run_10FAC_BAC_NOGRP_14:59:36_26_01_2024_copy/LLNEXT_microbiome_model_26_01_2024.hdf5"), load_interpol_Z = FALSE)
```
...
height_mm <- 297 # A third of the height of an A4 page in mm (≈99 mm)
...
# save as .pdf
ggsave("weights_16July2024.pdf", plot = p_weights, width = width_mm, height = height_mm, units = "mm")
```
...
source(here("run_10FAC_BAC_NOGRP_14:59:36_26_01_2024_copy/prep_phenotypes.R")) # script that prep phenotypes
data_phenos <- 
  phenos %>%
  rename(sample = NG_ID)
head(data_phenos)
...
```{r}
# helper functionto search through models
...
## Factor 1
mod <- find_matching_model_initial(Fac1_models, "birth_deliverybirthcard_mode_binary", "Factor1")

## Factor 2
mod <- find_matching_model_initial(Fac2_models, "birth_deliverybirthcard_mode_binary", "Factor2")

## Factor 3
mod <- find_matching_model_initial(Fac3_models, "infant_birthcard_feeding_mode_after_birth", "Factor3")

## Factor 4
mod <- find_matching_model_initial(Fac4_models, "mother_health_smoked_one_whole_year_p18", "Factor4")

## Factor 5
mod <- find_matching_model_initial(Fac5_models, "birth_deliverybirthcard_place_delivery_simple", "Factor5")

## Factor 6
mod <- find_matching_model_initial(Fac6_models, "infant_birthcard_feeding_mode_after_birth", "Factor6")

## Factor 8
mod <- find_matching_model_initial(Fac8_models, "infant_birthcard_feeding_mode_after_birth", "Factor8")

## Factor 10
mod <- find_matching_model_corrected(Fac10_models, "mother_birthcard_parity", "Factor10") # this is where the corrected phenotype had a lower p-value the the most significant association in the first series of models.
...
bp_shannon_fac <-
  ggplot(data_to_test %>% filter(!is.na(birth_deliverybirthcard_mode_binary)) %>% mutate(birth_deliverybirthcard_mode_binary = fct_relevel(birth_deliverybirthcard_mode_binary, "VG", "CS"))) +
...
# plot
ggplot(data_to_test) +
  geom_point(aes_string(x = "Factor3", y = "shannon", group = "infant_birthcard_feeding_mode_after_birth", color = "infant_birthcard_feeding_mode_after_birth")) +
  #geom_point() +
  geom_abline(slope = slope, intercept = intercept, color = "blue") +
  theme_minimal()
...
ggplot(data_to_test %>% filter(!is.na(infant_birthcard_feeding_mode_after_birth)) %>% mutate(infant_birthcard_feeding_mode_after_birth = fct_relevel(infant_birthcard_feeding_mode_after_birth, "BF", "MF", "FF"))) +
  geom_boxplot(aes_string(x = "infant_birthcard_feeding_mode_after_birth", y = "shannon")) +
  #geom_point() +
  #geom_abline(slope = slope, intercept = intercept, color = "blue") +
  theme_minimal()
