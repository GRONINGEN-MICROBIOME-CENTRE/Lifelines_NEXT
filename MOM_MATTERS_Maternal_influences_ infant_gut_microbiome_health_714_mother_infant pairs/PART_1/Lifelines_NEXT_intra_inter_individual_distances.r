...
# Authors: Sergio Andreu Sanchez, Trishla Sinha, Alexander Kurilshikov 
# Last update: 28th of September, 2025 
...
# Pertaining to Figure 1 E and F and Supplementary Table S20
...
Get_Pvalues_Permutations = function(dists_cons ,Which_type=  "mother:mother", Which_comparison = "P12-P28", n_permutations = 10000 ){
...
Get_Pvalues_Permutations(dists_cons, Which_type="infant:infant", Which_comparison = "M2-M3", n_permutations = 10000 )
...
for (type in unique_types) {
  ...
  for (time_comp in Comparisons_to_do) {
    ...
    result <- Get_Pvalues_Permutations(dists_cons, Which_type = type, Which_comparison = time_comp, n_permutations = 10000)
    results[[paste(type, time_comp, sep = "_")]] <- result
  }
}

final_results <- bind_rows(results, .id = "Type_TimeComparison")
# calculated the t statistics and to calculate the p value we permutate the labels 10000 times 


final_results$FDR<-p.adjust (final_results$Pvalue, method = "fdr")

# Resutls in Figure 1e & F and S20
write.table(final_results, "aitchison_distances_permutation_test_figure_1e.txt", sep = "\t", row.names = F)  
