...
infant_taxa_all_filt <- infant_taxa_all[, unique_counts >= 0.1*length(unique(infant_taxa_all$next_id_infant)) ] # Setting a 10% cut-off on prevalence (3rd august version) 
...
for (Timepoint in unique(metadata_infants$Timepoint_categorical)){
  ...
  Call_adonis(phenotypes, Phenotype_list=PhenosTest ,perm=10000, Distance=distance, covariates = "base") %>% mutate(Timepoint = Timepoint) %>% rbind(ResultsAdonis_base, .) -> ResultsAdonis_base
}


setwd("/Users/trishlasinha/Desktop/LLNEXT/Analysis/results/adonis_per_timepoint")
write.table(ResultsAdonis_base, "result_adonis_per_timepoint_infants_BY_MARGIN_10000_perm_cor_base_28_09_2025.txt", sep = "\t", row.names = F)

# Running model corrected for delivery mode 
ResultsAdonis_delivery = tibble()
for (Timepoint in unique(metadata_infants$Timepoint_categorical)){
  print(Timepoint)
  phenotypes<-metadata_infants %>% filter(Timepoint_categorical == Timepoint )
  PP=drop_phenotypes(phenotypes, non_NA_values = 60, minimum_variance = 8)
  PP %>% select(-one_of(not_to_test)) %>% colnames() -> PhenosTest
  Call_adonis(
    phenotypes,
    ...
    rbind(ResultsAdonis_delivery, .) -> ResultsAdonis_delivery
}
setwd("/Users/trishlasinha/Desktop/LLNEXT/Analysis/results/adonis_per_timepoint")
write.table(ResultsAdonis_delivery, "result_adonis_per_timepoint_infants_BY_MARGIN_10000_perm_cor_delivery_28_09_2025.txt", sep = "\t", row.names = F)

ResultsAdonis_feeding = tibble()
...
for (Timepoint in unique(metadata_infants$Timepoint_categorical)){
  ...
  Call_adonis(
    ...
    mutate(Timepoint = Timepoint) %>%
    rbind(ResultsAdonis_feeding, .) -> ResultsAdonis_feeding
}

setwd("/Users/trishlasinha/Desktop/LLNEXT/Analysis/results/adonis_per_timepoint")
write.table(ResultsAdonis_feeding, "result_adonis_per_timepoint_infants_BY_MARGIN_10000_perm_cor_delivery_feeding_28_09_2025.txt", sep = "\t", row.names = F)
