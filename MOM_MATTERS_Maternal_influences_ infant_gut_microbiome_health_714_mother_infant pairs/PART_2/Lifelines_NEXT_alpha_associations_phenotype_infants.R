### LIFELINES NEXT ASSOCIATION PHENOTYPES & ALPHA DIVERSITY INFANTS ####
...
results_base <- run_mmrm_analysis(shannon, phenos, 
...
results_delivery <- run_mmrm_analysis(shannon, phenos, 
                                      time = infant_metadata_cross_phenotypes_2$exact_age_months_at_collection, 
                                      time_cat = infant_metadata_cross_phenotypes_2$Timepoint_categorical, 
                                      NEXT_ID = infant_metadata_cross_phenotypes_2$NEXT_ID, 
...
results_delivery_breastfed <- run_mmrm_analysis(shannon, phenos, 
                                                time = infant_metadata_cross_phenotypes_2$exact_age_months_at_collection, 
                                                time_cat = infant_metadata_cross_phenotypes_2$Timepoint_categorical, 
                                                NEXT_ID = infant_metadata_cross_phenotypes_2$NEXT_ID, 
...
write.table(combined_results_cross, "Alpha_diversity_cross_phenotypes_results_23_09.txt", sep = "\t", row.names = F)
#combined_results_cross<-read.delim("Alpha_diversity_cross_phenotypes_results_23_09.txt")
...
infant_metadata_dynamic_phenotypes_2<- infant_metadata_dynamic_phenotypes_2[rownames(infant_metadata_dynamic_phenotypes_2) %in% rownames(shannon), ]
...
result_dynamic_alpha_base <- gam_function(shannon, infant_metadata_dynamic_phenotypes_2,metadata_columns, c("clean_reads_FQ_1","dna_conc", "BATCH_NUMBER"))
result_dynamic_alpha_cor_delivery <- gam_function(shannon, infant_metadata_dynamic_phenotypes_2,metadata_columns, c("clean_reads_FQ_1","dna_conc", "BATCH_NUMBER", "birth_deliverybirthcard_mode_binary"))
result_dynamic_alpha_cor_delivery_feeding<-gam_function(shannon, infant_metadata_dynamic_phenotypes_2,metadata_columns, c("clean_reads_FQ_1","dna_conc", "BATCH_NUMBER", "birth_deliverybirthcard_mode_binary","infant_ffq_ever_never_breastfed" ))
...
result_dynamic_alpha_base_re <- rename_except_dynamic(result_dynamic_alpha_base, "_cor_base")
result_dynamic_alpha_cor_delivery_re  <- rename_except_dynamic(result_dynamic_alpha_cor_delivery, "_cor_delivery")
result_dynamic_alpha_cor_delivery_feeding_re <- rename_except_dynamic(result_dynamic_alpha_cor_delivery_feeding , "_cor_delivery_breastfed")



combined_results_dynamic <- reduce(
  list(result_dynamic_alpha_base_re , result_dynamic_alpha_cor_delivery_re, result_dynamic_alpha_cor_delivery_feeding_re),
  full_join,
...
write.table(combined_results_dynamic, "Alpha_diversity_dynamic_phenotypes_results_all_23_09_2025.txt", sep = "\t", row.names = F)
