...
Fig_xgboost_LOO = function(X, Y, ratios = T, Do_logistic= F ){
  if (ratios == T){
    ...
    if (dim(X)[2] < 2 ){
      colnames(lrX) = 'Pred'
      if( (lrX[,1] %>% class()) == 'character'  ){
        #lrX[,1] = as.factor(lrX[,1]) %>% as.numeric()
        lrX = apply(lrX, 2, function(x){ as.factor(x) %>% as.numeric()  } )
      }
      
    }
    #X -> impute_zeros(X)
    #X  = vegan::decostand(X, 'clr')
  }
  ...
  for (i in 1:nrow(X_matrix)) {
    ...
    loo_predictions[i] <- predict(model, X_matrix[i, , drop = FALSE])
      ...
      ForModel = tibble( Pred = X_matrix[-i, 'Pred'], Y =   Y_vector[-i] )
      glm( Y ~ Pred,ForModel, family='binomial' ) -> model
      loo_predictions[i] <- predict(model, X_matrix[i, , drop = FALSE] %>% as.data.frame(),  type="response"  )
    }
  }
