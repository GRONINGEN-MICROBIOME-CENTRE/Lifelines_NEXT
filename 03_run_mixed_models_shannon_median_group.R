# 02_run_mixed_models_shannon_median_group.R
...
res_median_shannon <- extract_m2_results(model_results)
...
saveRDS(
  res_median_shannon,
  file = file.path(output_dir, "df_association_analysis_median_group_shannon_corrected_birth_mode_standardized_final.rds")
)

# BH correction
res_median_shannon_sig_BH <- res_median_shannon %>%
  group_by(component) %>%
  slice(1) %>%
  ungroup() %>%
...
saveRDS(
  res_median_shannon_sig_BH,
  file = file.path(output_dir, "df_significant_median_group_results_BH_corrected.rds")
)
