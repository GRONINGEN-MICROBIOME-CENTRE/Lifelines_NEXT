...
## Adding the number of discovered viruses per sample (BEFORE THE EXTENTION AND PRUNING)
...
ra_to_1kb_contigs <- as.data.frame(read_tsv("to_1kb_contigs.tsv"))
ra_to_all_vir <- as.data.frame(read_tsv("to_all_vir.tsv"))
ra_to_all_contigs <- as.data.frame(read_tsv("to_all_contigs.tsv"))
ra_to_ext_vir <- as.data.frame(read_tsv("to_ext_vir.tsv"))
ra_to_ext_prun_vir <- as.data.frame(read_tsv("to_ext_prun_vir.tsv"))

ra_to_all_contigs_to_1kb_contigs <- merge(x=ra_to_all_contigs, y=ra_to_1kb_contigs, by="sample_id", all = T)
ra_to_all_vir_to_ext_vir <- merge(x=ra_to_all_vir, y=ra_to_ext_vir, by="sample_id", all = T)
ra_to_all_contigs_to_1kb_contigs_to_all_vir_to_ext_vir <- merge(x=ra_to_all_contigs_to_1kb_contigs, y=ra_to_all_vir_to_ext_vir, by="sample_id", all = T)
ra_final <- merge(x=ra_to_all_contigs_to_1kb_contigs_to_all_vir_to_ext_vir, y=ra_to_ext_prun_vir, by="sample_id", all = T)

qc_assembly_vd_ra_stats <- merge(x=qc_assembly_vd_stats, y=ra_final, by="sample_id", all = T)

write.table(qc_assembly_vd_ra_stats, "C:\\Users\\Natal\\Documents\\UMCG\\amg_paper\\metadata\\qc_assembly_vd_ra_stats.tsv", sep='\t', row.names=F, col.names=T, quote=F)