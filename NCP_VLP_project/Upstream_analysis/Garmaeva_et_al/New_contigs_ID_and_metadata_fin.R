...
plasmids <- read.table(paste0(args[1], 'Prophage_pruning/', args[2], '_', args[3], '_plasmid_summary.tsv'), sep='\t', header=T)

prophage_prepruning <- read.table(paste0(args[1], 'Prophage_pruning/', args[2], '_', args[3], '_virus_summary.tsv'), sep='\t', header=T)
...
# Adding columns for the tools that are failed to find any viral sequences
# Define your list of expected column names
expected_columns <- c("Original_CID", "DeepVirFinder", "geNomad", "VirSorter2", "VIBRANT")
# Check which columns are missing
missing_columns <- setdiff(expected_columns, colnames(table_of_origin))
# Add missing columns and fill with 0
if (length(missing_columns) > 0) {
	for (col in missing_columns) {
		table_of_origin <- table_of_origin %>%
			mutate(!!col := 0)
	}
}
...
VLP_contigs_PD_metadata$Original_CID <- sub("(([^_]*_){9}[^_]*).*", "\\1", sub("\\|.*", "", VLP_contigs_PD_metadata$contig_id))

# here, the original contig length is extracted from its id, so it is also dependant on "_" in contig & sample id
VLP_contigs_PD_metadata$Original_length <- as.numeric(sapply(str_split(VLP_contigs_PD_metadata$Original_CID, "_"), `[[`, 8))
...
VLP_contigs_PD_metadata$New_CID <- paste0(args[4], "_", VLP_contigs_PD_metadata$Original_CID)
