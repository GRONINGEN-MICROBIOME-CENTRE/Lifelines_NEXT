...
VLP_counts <- as.data.frame(read_tsv('../VIR_DB/mapping/VLP_to_wo_nc99_der95_NCP/coverage_NCP/count_table.txt'))
names(VLP_counts)[1] <- "vOTUs"
row.names(VLP_counts) <- VLP_counts$vOTUs
VLP_counts$vOTUs <- NULL

VLP_contig_coverage <- as.data.frame(read_tsv('../VIR_DB/mapping/VLP_to_wo_nc99_der95_NCP/coverage_NCP/coverage_table.txt'))
...
# OUTPUT
##############################
write.table(RPKM_counts_VLP_filt, "RPKM_counts_VLP_wo_nc99_der95_bc99_NCP.txt", sep='\t')
