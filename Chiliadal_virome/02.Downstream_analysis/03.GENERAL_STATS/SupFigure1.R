...
mvg1b <- ETOF %>%
  ...
  geom_bar(position = "stack",
           stat = "identity") +
  labs(fill = "MIUViG category", y = "N vOTUs", x = "vOTUs by") +
  theme_minimal() +
...
ckv1b <- ETOF %>%
  ...
  geom_bar(position = "stack",
           stat = "identity") +
  labs(fill = "CheckV category", y = "N vOTUs", x = "vOTUs by") +
  theme_minimal() +
  theme(legend.position = "bottom",
...
nov1b <- ETOF %>%
  ...
  summarise(sum=n(), .groups = "drop") %>%
  mutate(vOTU_novelty = factor(vOTU_novelty, levels = c("novel", "described"), ordered = T)) %>%
...
FSUP1b <- ( mvg1b | ckv1b | gen1b  | tax1b |  lfs1b | simphost1b | bachost1b | nov1b) + plot_layout(guides = 'collect', axis_titles = 'collect') & theme(legend.position = "bottom")
...
FSUP1c <- (reads1c | csc) + plot_layout(guides = 'collect') & theme(legend.position = 'right', 
...
ggsave('05.PLOTS/03.GENERAL_STATS/Esmeta_VLP_MGS_stat.pdf',
       FSUP1c,  "pdf", width=20, height=10, units="cm", dpi = 300)
#############################################################
