...
betadisper <- map_dfr(phenos_select %>%
                        filter(cat_vs_num == "categorical" & analysis == "shaping") %>%
                        pull(new_name), 
                      function(pheno){
...
R2viz <- results_summary %>%
  ...
  labs(y = "R-sqaured", x="Technical feature") +
  ylim(0,0.54)

ggsave("05.PLOTS/07.Health_outcomes/VLP_shaping_factors_adonis.pdf",
       R2viz, "pdf", width=6, height=8, units="cm", dpi = 300)
...
FM <- ggplot(FM_list[["pcoa_plot_data"]], aes(x = V1, y = V2, color = !!sym(pheno))) +
  ...
  theme_bw() +
  labs(
    x = paste0("VLP, PC1: ", FM_list[["pvar"]][1], "%"),
    y = paste0("VLP, PC2: ", FM_list[["pvar"]][2], "%")) + 
...
ggsave("05.PLOTS/07.Health_outcomes/VLP_feeding_mode_PCoA.pdf",
       FM, "pdf", width=7, height=8, units="cm", dpi = 300)
...
DM <- ggplot(DM_list[["pcoa_plot_data"]], aes(x = V1, y = V2, color = !!sym(pheno))) +
  ...
  theme_bw() +
  labs(
    x = paste0("VLP, PC1: ", DM_list[["pvar"]][1], "%"),
    y = paste0("VLP, PC2: ", DM_list[["pvar"]][2], "%")) + 
  theme(axis.title = element_text(size=9),
        axis.text = element_text(size=8),
        legend.text = element_text(size=8),
        legend.title = element_text(size = 9),
...
ggsave("05.PLOTS/07.Health_outcomes/VLP_delivery_mode_PCoA.pdf",
       DM, "pdf", width=7, height=8, units="cm", dpi = 300)
