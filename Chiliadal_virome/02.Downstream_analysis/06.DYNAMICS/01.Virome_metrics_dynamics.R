...
ggplot(aes(Timepoint_new, value, fill = name)) +
  ggrastr::rasterise(geom_jitter(aes(color = name), position = position_jitterdodge(), size=0.05, alpha=1), dpi = 300)+
...
inf_mom_richness$p_adj <- p.adjust(inf_mom_richness$p.value, "BH")
...
rich_growth <- map_dfr(c("vir_richness_cf", "vir_richness_holo"), function(richness) {
  ...
  formula <- as.formula(paste0("log(",richness, "+1)", "~ Timepoint_new + (1|NEXT_ID)"))
...
model <- lmer(log(value + 1) ~ Timepoint_new*name + (1|NEXT_ID), data = growth_diff_om)
...
model <- lmer(log(prophage_richness + 1) ~ Timepoint_new + (1|NEXT_ID), data = smeta[smeta$Type == "K",])
...
inf_smeta_temp <- smeta %>%
  filter(Type == "K") %>%
  mutate(Timepoint_numeric = case_when( Timepoint_new == "M1" ~ 1,
                                        Timepoint_new == "M3" ~ 3,
                                        Timepoint_new == "M6" ~ 6,
                                        Timepoint_new == "M12" ~ 12))

# bacRichness is too much connected to time:
cor(log(inf_smeta_temp$bacRichness + 1),as.numeric(inf_smeta_temp$Timepoint_numeric), method = "pearson") # 0.61
bac_resid <- residuals(lmer(log(bacRichness + 1) ~ Timepoint_numeric + (1 | NEXT_ID), data = inf_smeta_temp))
cor(unname(bac_resid),as.numeric(inf_smeta_temp$Timepoint_numeric), method = "pearson") # -1.596144e-14

delta_growth_driver <- summary(lmer(log(delta + 1) ~ Timepoint_numeric + log(prophage_richness + 1) + bac_resid + (1 | NEXT_ID), 
                                    data = inf_smeta_temp))$coefficients
...
proph_growt <- smeta %>%
  ...
  ggrastr::rasterise(geom_jitter(position = position_jitterdodge(), size=0.02, alpha=1, color = "firebrick"), dpi = 300) +
...
ggsave("05.PLOTS/06.DYNAMICS/Prophages_growth.pdf", proph_growt, "pdf", width = 6, height=7, dpi = 300, units = "cm")
...
ggsave("05.PLOTS/06.DYNAMICS/BacRichness_growth.pdf", bac_growt, "pdf", width = 6, height=7, dpi = 300, units = "cm")
...
summary(smeta$euk_perc[smeta$Type == "K"])
...
diff_host_richness_dynamics <- map_dfr(diff_richness, function(richness){
            ...
            formula <- paste("log(",richness, " + 1) ~ Timepoint_new + (1|NEXT_ID)")
...
ggsave("05.PLOTS/06.DYNAMICS/Prok_euk_richness_growth_lines.pdf",
       prok_euks_lines, "pdf", width = 11, height=10, dpi = 300, units = "cm")
#############################################################
...
ggsave("05.PLOTS/06.DYNAMICS/Lifestyle_richness_by_time.pdf", lifestyle_richness, "pdf", width = 7, height=10, dpi = 300, units = "cm")
...
smeta <- smeta %>%
  mutate(temp_to_lytic_rich_log = log10( (lytic_richness +1)/(temperate_richness + 1)),
         temp_to_lytic_ab_log = log10((lytic_RAb+0.0009)/(temperate_RAb + 0.0009) + 0.01/2))

temp_to_lytic_rich_log <- mixed_model_tukey(smeta, 
                                            "temp_to_lytic_rich_log ~ Type + (1|NEXT_ID)",
                                            "Type") 

writexl::write_xlsx(temp_to_lytic_rich_log, '07.RESULTS/Phage_lifestyle_mother-infant.xlsx')
