...
# Select outcome 
pathway_1 <- pathways[match(rownames(infant_metadata_cross_phenotypes_2), rownames(pathways)),]
...
results_1<-run_mmrm_analysis(pathway_1, phenos,time = infant_metadata_cross_phenotypes_2$exact_age_months_at_collection, time_cat = infant_metadata_cross_phenotypes_2$Timepoint_categorical, NEXT_ID = infant_metadata_cross_phenotypes_2$NEXT_ID, covariates =  covariates_normalized )
...
setwd("~/Desktop/LLNEXT/Analysis/results/pathways/infants/")


write.table(results_trait, "pathways_mmrm_trait_cross_sectional_infants.txt", sep = "\t", row.names = F)
write.table(results_trait_time_interaction, "pathways_mmrm_trait_int_time_cross_sectional_infants.txt", sep = "\t", row.names = F)
