### LIFELINES NEXT GUT MICROBIOME ANALYSIS ALPHA DIVERSITY MOTHERS ####
...
results_1<-run_mmrm_analysis(shannon, phenos,time = mother_metadata_cross_phenotypes_2$exact_age_months_at_collection, time_cat = mother_metadata_cross_phenotypes_2$Timepoint_categorical, NEXT_ID = mother_metadata_cross_phenotypes_2$NEXT_ID, covariates =  covariates_normalized )
...
setwd("~/Desktop/LLNEXT/Analysis/results/alpha_diversity")

write.table(results_trait, "alpha_diversity_mmrm_trait_cross_sectional_mothers.txt", sep = "\t", row.names = F)
write.table(results_trait_time_interaction, "alpha_diversity_mmrm_trait_time_interaction_cross_sectional_mothers.txt", sep = "\t", row.names = F)
