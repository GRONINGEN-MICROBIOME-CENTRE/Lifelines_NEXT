...
infant_cross_selection <- cross_selection %>% 
  filter(only_run_vaginal_delivery_STRICT == 1) %>% 
  na.omit()
...
results_trait<-as.data.frame(results_1$trait)
results_place_delivery<-results_trait[results_trait$trait=="birth_deliverybirthcard_place_delivery_simple",]
results_place_delivery$FDR<-p.adjust (results_place_delivery$P, method = "fdr")
...
write.table(results_trait, "taxa_mmrm_trait_VAGINAL_ONLY_STRICT_cross_sectional_infants.txt", sep = "\t", row.names = F)
write.table(results_trait_time_interaction, "taxa_mmrm_trait_int_time_VAGINAL_ONLY_cross_sectional_infants.txt", sep = "\t", row.names = F)
...
long_labor<- all %>%
  filter(!is.na(mother_birthcard_del_complications_long_labor_O63))

# Top results 
ggplot(long_labor, aes(Timepoint_categorical, y=Veillonella_parvula.t__SGB6939, fill=mother_birthcard_del_complications_long_labor_O63, color=mother_birthcard_del_complications_long_labor_O63)) +
  scale_fill_manual(values = c("#54b7a6", "#fa2b13"), labels = c("No", "Yes")) +
  scale_color_manual(values = c("#54b7a6", "#fa2b13"), labels = c("No", "Yes")) +
  geom_boxplot(alpha=0.4, outlier.colour = NA) +
  geom_point(alpha=0.6,
             position = position_jitterdodge(jitter.width = 0.3, jitter.height = 0)) +
  ggtitle("") +
  theme_bw() + labs(x="", y = "Abundance Veillonella parvula ", fill="", color="") +
  theme(
...
epidural<- all %>%
  filter(!is.na(mother_deliverybirthcard_med_del_epidural))



ggplot(epidural, aes(Timepoint_categorical, y=Enterococcus_casseliflavus.t__SGB7952, fill=mother_deliverybirthcard_med_del_epidural, color=mother_deliverybirthcard_med_del_epidural)) +
  scale_fill_manual(values = c("#54b7a6", "#fa2b13"), labels = c("No", "Yes")) +
  scale_color_manual(values = c("#54b7a6", "#fa2b13"), labels = c("No", "Yes")) +
  geom_boxplot(alpha=0.4, outlier.colour = NA) +
  geom_point(alpha=0.6,
             position = position_jitterdodge(jitter.width = 0.3, jitter.height = 0)) +
  ggtitle("") +
  theme_bw() + labs(x="", y = "Abundance Enterococcus_casseliflavus ", fill="", color="") +
  theme(
    axis.title.y = element_text(color="black", size=18),
    axis.text.y = element_text(size=12),
    axis.text.x = element_text(size=12, angle = 60, hjust = 1))
