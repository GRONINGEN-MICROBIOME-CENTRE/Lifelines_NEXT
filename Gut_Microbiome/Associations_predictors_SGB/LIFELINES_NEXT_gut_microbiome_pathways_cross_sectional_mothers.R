### LIFELINES NEXT GUT MICROBIOME ANALYSIS PATHWAYS MOTHERS ####
...
# Select outcome 
pathways <- pathways[match(rownames(mother_metadata_cross_phenotypes_2), rownames(pathways)),]
...
results_1<-run_mmrm_analysis(pathways, phenos,time = mother_metadata_cross_phenotypes_2$exact_age_months_at_collection, time_cat = mother_metadata_cross_phenotypes_2$Timepoint_categorical, NEXT_ID = mother_metadata_cross_phenotypes_2$NEXT_ID, covariates =  covariates_normalized )
...
setwd("~/Desktop/LLNEXT/Analysis/results/pathways/mothers/")


write.table(results_trait, "pathways_mmrm_trait_cross_sectional_mothers.txt", sep = "\t", row.names = F)
write.table(results_trait_time_interaction, "pathways_mmrm_trait_int_time_cross_sectional_mothers.txt", sep = "\t", row.names = F)

all<-merge(mother_metadata_cross_phenotypes_2, pathways, by="row.names")
