#!/bin/bash
#SBATCH --job-name=CheckV_derep_LLNEXT_viruses
#SBATCH --output=CheckV_derep_LLNEXT_viruses.out
#SBATCH --mem=20gb
...
# Extract only comparisons between LLNEXT genomes from the blast results
cat $blast_db | grep -Ev "MGV|GPD|Gulyaeva|Benler|Shah|IMG_VR|ELGV|RefSeq|Guerin|Yutin" > BLASTN_RESULTS/LLNEXT_viruses_blast.tsv 
...
python /scratch/hb-llnext/PentaChiliadal/6_DEREPLICATION_vOTUs/anicalc.py -i BLASTN_RESULTS/LLNEXT_viruses_blast.tsv  -o LLNEXT_DEREPLICATION/LLNEXT_viruses_ani.tsv

# Perform UCLUST-like clustering (99% ANI + 95% AF):
python /scratch/hb-llnext/PentaChiliadal/6_DEREPLICATION_vOTUs/aniclust.py --fna LLNEXT_viral_genomes.fa --ani LLNEXT_DEREPLICATION/LLNEXT_viruses_ani.tsv --out LLNEXT_DEREPLICATION/LLNEXT_viral_genome_clusters.tsv --min_ani 99 --min_tcov 95 --min_qcov 0
...
python /scratch/hb-llnext/PentaChiliadal/6_DEREPLICATION_vOTUs/aniclust.py --fna LLNEXT_dedup_viral_genomes.fa --ani LLNEXT_DEREPLICATION/LLNEXT_viruses_ani.tsv --out LLNEXT_DEREPLICATION/LLNEXT_vOTU_clusters.tsv --min_ani 95 --min_tcov 85 --min_qcov 0
